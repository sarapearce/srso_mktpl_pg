...
function revealOverlay() {
    ...
    //animation for revealing what is under overlay
    $(window).scroll(function () {
	if ($(this).scrollTop() > 100) {
//	    var y = $(this).position().top;
	    $('.totop').animate({up: '100px'});
	}
    });
}

//the scroll animation for the 3 city example sections
function Scroll() {
    var lastScrollTop = 0;
    var backgroundImages = $('.bg-image');
...
//	stickyNavToggle(); // Sticky nav toggle
	revealOverlay(); //lift white overlay in sync with scroll
    });
}
;
...
function strToArray(str) {
	...
	document.createElement('div').innerHTML = err.message;
    }
...
//
//// white overlay sliding up effect
//$(window).scroll(function() {
//if ($(this).scrollTop() > 100) {
//    $('.totop').stop().animate({ up: '0px' });
//});

//need to add in HTML .totop class to the overlay 
